...
static struct Command commands[] = {
	...
	{"continue", "During a breakpoint exception, continues executing the code.", continue_breakpoint},
	{"si", "During a breakpoint exception, single step through each assembly step.", single_step},
};

/***** Implementations of basic kernel monitor commands *****/

int
continue_breakpoint(int argc, char **argv, struct Trapframe *tf) {

	// Make sure that single stepping is disabled by setting trap flag to 0. 
	tf->tf_eflags = (tf->tf_eflags) & (~FL_TF);
	
	// Return to the current environment, which should be running.
	if(curenv && curenv->env_status == ENV_RUNNING) {
		env_run(curenv);
		panic("continue_breakpoint in monitor.c: Should have jumped back to code that called the breakpoint. ");
	} else {
		cprintf("Warning: You are trying to continue to a different environment (not the currently running environment). Continue is meant to be used with breakpoint. \n");
		return -1; 
	}
...
single_step(int argc, char **argv, struct Trapframe *tf) {
	...
	// Turn on the trap flag in eflags. The trap flag is a debugging mechanism implemented by the processor that throws int $1 after every instruction. 
	tf->tf_eflags = (tf->tf_eflags) | (FL_TF); 
	
	// Return to the current environment, which should be running.
	if(curenv && curenv->env_status == ENV_RUNNING) {
		env_run(curenv);
		panic("single_step in monitor.c: Should have jumped back to code that called the breakpoint. ");
	} else {
		cprintf("Warning: You are trying to continue to a different environment (not the currently running environment). Continue is meant to be used with breakpoint. \n");
		return -1; 
	}
